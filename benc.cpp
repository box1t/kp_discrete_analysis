    ...
    std::vector<std::string> preprocess_text(const std::string& input_text) {
        ...
        for (char ch : input_text) {
            if (std::isspace(static_cast<unsigned char>(ch))) {
                ...
                text_piece += static_cast<char>(std::tolower(static_cast<unsigned char>(ch)));
            }
        }
        ...
        auto preprocessEnd = std::chrono::high_resolution_clock::now(); // Конец замера времени
...
public:
               ...
               TextPreprocessor& text_preprocessor)
    {
        for (size_t i = 0; i < documents.size(); ++i) {
            ...
            // Токенизируем (разбиваем на text_pieces)
            std::vector<std::string> text_pieces = text_preprocessor.preprocess_text(doc);
        ...
        // Считаем логарифм априорных вероятностей
        int total_docs = static_cast<int>(documents.size());
        for (const auto& [class_label, doc_count] : documents_in_classes_counter) {
            log_prior_prob_for_classes[class_label] =
                std::log(static_cast<double>(doc_count)) - std::log(static_cast<double>(total_docs));
        }
    }

    // Передаём сюда text_preprocessor как параметр, чтобы не создавать новый в каждом вызове
    std::string classify(const std::string& document,
                         TextPreprocessor& text_preprocessor) const
    {
        // Токенизируем (получаем text_pieces)
        std::vector<std::string> text_pieces = text_preprocessor.preprocess_text(document);

        // Считаем, сколько раз каждая text_piece встретилась в данном документе
        ...
        // Перебираем все классы, чтобы найти, для какого класса score больше
        for (const auto& [class_label, _] : documents_in_classes_counter) {
            double score = log_prior_prob_for_classes.at(class_label);

            // Учитываем вклад каждой text_piece
            for (const auto& [text_piece, freq_in_doc] : text_pieces_counts_in_doc) {
                int text_piece_count_for_class = 0;
                // Сколько раз данная text_piece встретилась в тренировочном наборе для класса class_label
                if (word_frequencies_in_classes.count(text_piece) &&
                    word_frequencies_in_classes.at(text_piece).count(class_label))
                {
                    text_piece_count_for_class = word_frequencies_in_classes.at(text_piece).at(class_label);
                }

                double numerator   = text_piece_count_for_class + smoothing_factor;
                double denominator = total_words_in_classes_counter.at(class_label) + smoothing_factor;
                // Умножаем логарифм вероятности на количество вхождений text_piece в документе
...
int main() {
    ...
    // Считываем остаток строки, чтобы не мешал при дальнейшем чтении
    std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');

    // Вектор документов и их классов для обучения
    ...
    // Классифицируем test_counter документов и замеряем время
    auto predictStart = std::chrono::high_resolution_clock::now();
    for (int i = 0; i < test_counter; ++i) {
        std::string testDocument;
        std::getline(std::cin, testDocument);

        // Передаём тот же самый text_preprocessor
        std::cout << classifier.classify(testDocument, text_preprocessor) << '\n';
    }
